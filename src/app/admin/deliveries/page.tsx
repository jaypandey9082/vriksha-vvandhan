...
export default async function DeliveryCenterPage({ searchParams }: { searchParams: Promise<DeliveryFilters & { result?: string }> }) {
  ...
  return <>
    ...
    <section className="admin-panel"><div className="admin-panel__heading"><div><p>Private operations</p><h2>Recent delivery records</h2></div></div>
      <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Participant</th><th>Delivery</th><th>Status</th><th>Submitted</th><th>Generated / sent</th><th>Attempts</th><th>Action</th></tr></thead><tbody>
        {data.rows.map((row) => <tr key={`${row.deliveryType}-${row.id}`}><td><Link href={`/admin/submissions/${row.submissionId}`}>{row.displayName ?? "Participant"}</Link><small>{row.guardianNumber ? `Guardian #${row.guardianNumber}` : "No Guardian number"}</small></td><td>{row.kind.replaceAll("_", " ")}</td><td><span className={`status-badge status-badge--${row.status}`}>{row.status.replaceAll("_", " ")}</span>{row.lastErrorCode && <small>{row.lastErrorCode.replaceAll("_", " ")}</small>}</td><td>{dateLabel(row.submittedAt)}</td><td>{dateLabel(row.deliveredAt)}</td><td>{row.attemptCount}</td><td><div className="admin-delivery-actions">
