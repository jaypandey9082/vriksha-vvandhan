...
Trash immediately hides the row and removes public variants through the Storage API while retaining the original and review thumbnail for recovery. Published restore generates a new immutable version before restoring public visibility. Permanent deletion and bounded demo cleanup remove the original, review thumbnail, public variants and any certificate through the API before deleting the database record. Section 5 will create the private certificate object.
