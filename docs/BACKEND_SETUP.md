...
`db:reset` applies every migration to a fresh local database and runs the intentionally participant-free seed. Bucket seeding reads the three definitions from `supabase/config.toml`. `db:types:check` generates an ignored temporary file and fails on drift instead of overwriting the committed linked snapshot.
